...
12. **Every endpoint declares its response type in the OpenAPI spec.** Use `@ApiOkResponse({ type: ResponseDto })` (or `@ApiCreatedResponse` for `POST`s that 201) on every controller method. Create a `*-response.dto.ts` next to existing request DTOs — plain class fields with accurate `?: T` for nullable fields; the `@nestjs/swagger` CLI plugin (enabled in `nest-cli.json`) auto-adds `@ApiProperty` from the TypeScript types. Returning a bare entity is forbidden — entities are storage shapes, response DTOs are wire shapes, and the spec must describe the wire. Pattern reference: `src/community/dto/rtp-response.dto.ts` + `src/community/rtp.controller.ts`. Reason: `tennis-miami` and `tennis-miami-web` generate their typed API clients from `/api/docs-json` via `swagger_parser` (Dart) and `openapi-zod-client` (TS); an undocumented response shape generates a client with `any`/`void`, which is how the May 2026 RTP/profile silent-drift bugs reached production.

13. **DTO class names are domain-prefixed.** When a DTO class would otherwise share its name with one in another module (e.g. `Location`, `CancellationPolicy`, `RepeatingData`), prefix it with the domain it belongs to: `SessionLocation`, `BookingCancellationPolicy`, `ProjectRepeatingData`. The OpenAPI spec uses class names directly as schema names, and `swagger_parser` / `openapi-zod-client` suffix collisions with `2`, `3`, `4` — generated clients then have ambiguous `Location2` / `Location3` types whose meaning depends on which module's spec entry got walked first. Naming up front is free; renaming after a thousand call sites have imported `Location2` is not. This applies to BOTH request and response DTOs, and to TypeORM entity names exposed via the spec. If you're unsure of the right prefix, use the parent NestJS module name (e.g. `arenna.bookings.controller.ts` → `BookingCancellationPolicy`).
